...
在「设置 > 检查点」里有两项和模型相关的配置，它们只影响**模型名反查与模型管理**，不会修改你的图片：

### 1. 模型根目录

把你本地 Stable Diffusion 模型文件夹（例如 AUTOMATIC1111 的 `models\Stable-diffusion`）设为**模型根目录**后，SimpaiViewer.NET 会扫描目录里的 `.ckpt` / `.safetensors` 等模型文件，计算它们的哈希值，并和图片来自元数据中的模型哈希进行匹配。

效果：

* 元数据面板里原本只显示哈希的地方，会显示成你能看懂的**模型文件名**；
* 工具栏的「模型」视图会列出所有本地模型，并统计每个模型生成了多少张图；
* 搜索时可以用 `model:` 按模型名查找。

### 2. A1111 缓存（cache.json）

AUTOMATIC1111 会在 `cache.json` 里记录它加载过的模型名称与哈希（包括旧版 hash 和 SHA256）。如果你的图片元数据里只有模型哈希、没有模型名，设置 `cache.json` 路径后，SimpaiViewer.NET 就能通过哈希反查出模型名，并支持 `model:` / `model_hash:` 搜索。

> 简言之：**模型根目录**让程序认识你电脑上的模型文件；**cache.json** 让程序利用 A1111 已有的哈希-名称映射。两者都不必填，但填上后模型信息会更完整。

---

## 十二、常见问题（FAQ）

以下内容翻译自 [DiffusionToolkit 上游 FAQ](https://github.com/RupertAvery/DiffusionToolkit#faq)。

### 如何查看图片的元数据（PNGInfo）？

确保预览面板可见，在缩略图区按 `I`，或把焦点放到预览面板后按 `I`，即可显示/隐藏元数据。也可以点击预览面板右下角的眼睛图标。

### 什么时候需要「重建元数据」？

只有当新版本支持了你已有图片中的某种新元数据格式时才需要运行。重建元数据**不会**影响你已设置的收藏、评分、NSFW 等标签。

### 我可以把图片移动到别的文件夹吗？

可以，但建议仍在本软件内完成：选中图片后 **右键 > 移动**。这样 SimpaiViewer.NET 会同步更新数据库中的路径，保留收藏、评分、NSFW 等 Toolkit 元数据。

如果你用资源管理器或其他工具移动，即使仍在已扫描目录内，再次扫描时程序会认为原文件被删除、新文件被加入，你将丢失这些 Toolkit 元数据。

### 设置和数据库存在哪里？

默认保存在 `%APPDATA%\SimpaiViewer`。

删除该数据库会丢失收藏、评分、相册、自定义标签等 Toolkit 数据，但你的原图文件不会被删除。

---

## 十三、支持作者

如果 SimpaiViewer.NET 对你有帮助，欢迎打赏支持后续开发。

> **建议**：面向国内用户，**微信二维码**是最直接、最熟悉的方式，截图保存到仓库（例如 `docs/images/wechat-donate.png`）后用 Markdown 引用即可；如果你已有收款链接（如爱发电、Ko-fi），也可以放一个**按钮式链接**作为补充。两者不冲突，二维码为主、按钮为辅会更方便。
